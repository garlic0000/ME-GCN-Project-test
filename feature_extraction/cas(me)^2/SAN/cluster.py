...
def main():
    ...
    assert len(cls_train_dataset.classes) == 4, 'There should have 4 kinds of classes instead of : {:}'.format(
        cls_train_dataset.classes)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (inputs, target) in enumerate(cls_train_loader):
            ...
            if i % args.print_freq == 0 or i + 1 == len(cls_train_loader):
                print_log(
                    ' [Train={:03d}] [{:}] [{:3d}/{:3d}] accuracy : {:.1f}, loss : {:.4f}, input:{:}, output:{:}'.format(
                        epoch, time_string(), i, len(cls_train_loader), top1.avg, losses.avg, inputs.size(),
                        output.size()), log)
        ...
        for i, (inputs, target) in enumerate(val_loader):
            ...
            if i % args.print_freq_eval == 0 or i + 1 == len(val_loader):
                print_log(
                    ' [Evalu={:03d}] [{:}] [{:3d}/{:3d}] accuracy : {:.1f}, loss : {:.4f}, input:{:}, output:{:}'.format(
                        epoch, time_string(), i, len(val_loader), top1.avg, losses.avg, inputs.size(), output.size()),
                    log)
    ...
    for iL in range(args.n_clusters):
        ...
        print_log(
            '{:} [{:2d} / {:2d}] has {:4d} / {:4d} -> {:4d} = {:.2f} images'.format(time_string(), iL, args.n_clusters,
                                                                                    indexes.size, len(data),
                                                                                    len(filtered_index),
                                                                                    indexes.size * 1. / len(data)), log)
